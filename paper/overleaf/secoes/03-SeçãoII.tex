...
A Figura~\ref{fig:modelo_sistema} ilustra o modelo geral de segurança adotado neste trabalho. A estação rádio base transmite um sinal $x$, que é recebido por Alice, Bob e pelo invasor Eve através de canais sem fio distintos. Conforme o modelo apresentado em~\eqref{eq:canal_basico}, o sinal recebido por cada entidade pode ser descrito por $y_i = h_i x + n_i$, em que $i \in \{A, B, E\}$ identifica Alice, Bob e Eve; $h_i$ denota o coeficiente complexo de desvanecimento do canal entre a estação rádio base e cada receptor; e $n_i$ corresponde ao ruído \ac{AWGN} observado em cada dispositivo.
...
Neste trabalho, adota-se o modelo de desvanecimento Rayleigh plano, apropriado para cenários sem linha de visada direta (\ac{NLoS}) em ambientes urbanos densos e internos, típicos de aplicações \ac{IoT}~\cite{tse_viswanath}. Sob esse modelo, o coeficiente de canal $h$ é uma variável aleatória complexa gaussiana $h \sim \mathcal{CN}(0,1)$, cuja envoltória $|h|$ segue distribuição de Rayleigh com função densidade de probabilidade (\ac{PDF}) dada por
%
\begin{equation}
f_{|h|}(r) = r \, e^{-r^2/2}, \quad r \geq 0,
\label{eq:pdf_rayleigh}
\end{equation}
%
e cuja fase $\angle h$ é uniformemente distribuída em $[0, 2\pi]$. O parâmetro de desvanecimento (escala) é normalizado para potência unitária, ou seja, $\mathbb{E}[|h|^2] = 1$, de modo que a \ac{SNR} média é determinada exclusivamente pela potência do ruído $\sigma_n^2 = \mathbb{E}[|n|^2]$ segundo $\text{SNR} = 1/\sigma_n^2$. Esse modelo captura as flutuações rápidas de amplitude características de canais multipercurso e é amplamente utilizado em análises teóricas e validações experimentais de sistemas \ac{PKG}~\cite{mathur_pks}.
...
Considerando um modelo de canal com desvanecimento plano e quase-estático, o sinal recebido pelo dispositivo $i \in \{A,B\}$ pode ser modelado conforme~\eqref{eq:canal_basico} na forma dependente do tempo: $y_i(t) = h_i(t)\,x(t) + n_i(t)$, em que $x(t)$ representa o sinal de sondagem transmitido pela estação rádio base ou ponto de acesso, $h_i(t)$ denota o coeficiente complexo do canal entre a infraestrutura e o dispositivo $i$, e $n_i(t)$ corresponde ao ruído \ac{AWGN} no receptor.
...
De forma geral, a quantização pode ser representada por~\cite{bloch_wireless_security}
\begin{equation}
...
Neste trabalho, adota-se \textbf{quantização antipodal} diretamente sobre os sinais demodulados \ac{BPSK} ou \ac{QPSK}. Para \ac{BPSK}, os símbolos recebidos são mapeados no conjunto $\{+1, -1\}$, e a quantização binária é realizada com limiar $\tau = 0$ (zero-crossing), definida como~\cite{proakis_digital}
\begin{equation}
b_i(k) =
\begin{cases}
1, & \text{Re}\{h_i(k) x(k)\} \geq 0, \\
0, & \text{Re}\{h_i(k) x(k)\} < 0,
\end{cases}
\end{equation}
em que $\text{Re}\{\cdot\}$ denota a parte real do sinal demodulado. Para \ac{QPSK}, a quantização é aplicada independentemente às componentes em fase (I) e quadratura (Q), gerando 2 bits por símbolo, cada um quantizado com limiar zero sobre suas respectivas componentes.

A escolha da quantização antipodal com limiar $\tau = 0$ justifica-se por três razões principais: (i) \textbf{otimalidade estatística}: para sinais com distribuição simétrica em torno de zero (caso de canais Rayleigh com média nula), o limiar zero maximiza a entropia dos bits gerados~\cite{mathur_pks}; (ii) \textbf{simplicidade de implementação}: não requer estimação de parâmetros estatísticos do canal ou ajuste adaptativo de limiares, reduzindo custo computacional; (iii) \textbf{robustez}: evita viés sistemático introduzido por limiares fixos não-nulos que poderiam favorecer um dos níveis lógicos.

Estratégias de limiarização adaptativa, como introdução de zona morta (\textit{guard-band}) ao redor do limiar de decisão, podem ser empregadas para descartar amostras cuja magnitude esteja próxima de $\tau$, aumentando robustez contra ruído~\cite{proakis_digital}. Embora essa técnica possa reduzir a taxa de discordância inicial (\ac{BMR}) entre Alice e Bob, também diminui a taxa efetiva de geração de bits devido ao descarte de amostras incertas. O Experimento~7 (Seção~\ref{sec:results}) demonstra que, no sistema proposto baseado em correlação espacial, a utilização de \textit{guard-band} não é necessária para garantir segurança, podendo-se operar com limiar simples $\tau = 0$ sem comprometimento da proteção contra espionagem passiva.
...
Ao receber $\boldsymbol{\sigma}$, Alice calcula $\mathbf{c}' = \boldsymbol{\sigma} \oplus \mathbf{b}_A = \mathbf{c} \oplus \mathbf{e}$, onde $\mathbf{e} = \mathbf{b}_A \oplus \mathbf{b}_B$ representa o vetor de erros entre as sequências~\cite{bloch_wireless_security}. Aplicando o algoritmo de decodificação \ac{BCH}, Alice estima
\begin{equation}
\hat{\mathbf{c}} = \text{BCH}_{\text{dec}}(\mathbf{c}'),
\label{eq:decod_bch}
\end{equation}
recuperando $\hat{\mathbf{c}} = \mathbf{c}$ caso o peso de Hamming $||\mathbf{e}||_H \leq t$~\cite{lin_costello_ecc}. A sequência reconciliada é então obtida por
\begin{equation}
\hat{\mathbf{b}}_A = \boldsymbol{\sigma} \oplus \hat{\mathbf{c}} = \mathbf{b}_B.
\label{eq:reconciliada}
\end{equation}
...
\textbf{Geração de Espaço Amostral Eficiente:} Um aspecto prático importante é a geração eficiente do espaço amostral necessário para validação estatística do sistema. Em simulações de Monte Carlo, é necessário gerar $N_{\text{bits}}$ bits compartilhados para cada realização. Como o código \ac{BCH}(127,64,10) opera em blocos de $n=127$ bits, a geração de sequências de comprimento arbitrário ($N_{\text{bits}}$ pode não ser múltiplo de 127) requer processamento de blocos parciais. Para evitar descarte excessivo e garantir taxa efetiva máxima, adota-se a seguinte estratégia: (i) calcula-se o número de blocos completos $N_{\text{blocos}} = \lfloor N_{\text{bits}} / n \rfloor$; (ii) processa-se cada bloco completo via~\eqref{eq:sindrome}-\eqref{eq:reconciliada}; (iii) para o bloco parcial residual ($N_{\text{resto}} = N_{\text{bits}} \mod n$ bits), aplica-se padding com zeros até completar um bloco de 127 bits, reconcilia-se, e descartam-se os bits de padding, retendo apenas os $N_{\text{resto}}$ bits úteis. Esse procedimento garante aproveitamento máximo dos bits gerados sem introduzir viés estatístico, sendo fundamental para obtenção de estimativas acuradas de \ac{BMR} e \ac{KDR} nos experimentos.

O Algoritmo~\ref{alg:reconciliacao} sintetiza o procedimento completo de reconciliação \textit{code-offset} BCH empregado neste trabalho.

\begin{algorithm}[htbp]
\caption{Reconciliação Code-Offset BCH}
\label{alg:reconciliacao}
\begin{algorithmic}[1]
\REQUIRE Sequências $\mathbf{b}_A, \mathbf{b}_B \in \{0,1\}^N$, código BCH$(n,k,t)$
\ENSURE Sequências reconciliadas $\hat{\mathbf{b}}_A = \mathbf{b}_B$
\STATE \textbf{Bob:}
\STATE Gera vetor aleatório $\mathbf{r} \in \{0,1\}^k$
\STATE Codifica: $\mathbf{c} \leftarrow \text{BCH}_{\text{enc}}(\mathbf{r})$ \COMMENT{$\mathbf{c} \in \{0,1\}^n$}
\STATE Calcula síndrome: $\boldsymbol{\sigma} \leftarrow \mathbf{b}_B \oplus \mathbf{c}$
\STATE Transmite $\boldsymbol{\sigma}$ via canal público para Alice
\STATE
\STATE \textbf{Alice:}
\STATE Recebe $\boldsymbol{\sigma}$
\STATE Calcula: $\mathbf{c}' \leftarrow \boldsymbol{\sigma} \oplus \mathbf{b}_A$
\STATE Decodifica BCH: $\hat{\mathbf{c}} \leftarrow \text{BCH}_{\text{dec}}(\mathbf{c}')$
\IF{decodificação bem-sucedida ($||\mathbf{c}' \oplus \hat{\mathbf{c}}||_H \leq t$)}
    \STATE $\hat{\mathbf{b}}_A \leftarrow \boldsymbol{\sigma} \oplus \hat{\mathbf{c}}$
    \STATE \textbf{return} $\hat{\mathbf{b}}_A$ \COMMENT{Alice e Bob compartilham $\mathbf{b}_B$}
\ELSE
    \STATE \textbf{return} FALHA \COMMENT{Reconciliação falhou, repetir processo}
\ENDIF
\end{algorithmic}
\end{algorithm}
