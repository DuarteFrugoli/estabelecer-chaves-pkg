...
A Figura~\ref{fig:exp01_snr} apresenta as curvas de \ac{BMR} e \ac{KDR} em função da \ac{SNR}, demonstrando o impacto da reconciliação \ac{BCH}. Observa-se que o \ac{BMR} decresce continuamente com a \ac{SNR}, enquanto o \ac{KDR} apresenta decaimento exponencial mais acentuado, atingindo valores nulos a partir de $13$--$15$~dB. A Tabela~\ref{tab:exp01_snr} apresenta os valores numéricos obtidos para pontos selecionados.
...
\caption{Impacto da SNR no desempenho do sistema. (a) BMR decresce continuamente com SNR. (b) KDR apresenta decaimento exponencial, atingindo valores nulos em 13--15~dB, demonstrando eficácia da reconciliação BCH e viabilidade para enlaces típicos de IoT.}
\label{fig:exp01_snr}
\end{figure*}
...
A Tabela~\ref{tab:exp02_modulacao} apresenta a comparação entre as duas modulações para valores selecionados de \ac{SNR}. Os resultados indicam desempenho muito similar entre \ac{BPSK} e \ac{QPSK}, com diferenças marginais de \ac{BMR} e \ac{KDR} que se tornam desprezíveis em regime de alta \ac{SNR}.
...
O quarto experimento mede o tempo de execução das principais operações do sistema para diferentes códigos \ac{BCH}. A Figura~\ref{fig:exp04_complexidade} apresenta graficamente os tempos de codificação e decodificação, enquanto a Tabela~\ref{tab:exp04_complexidade} apresenta os valores numéricos completos.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figuras/exp04_analise_complexidade_01_20260212_021634.png}
\caption{Tempo de codificação versus decodificação para diferentes códigos BCH. O código BCH(127,64,10) apresenta tempo total de 0.36~ms, demonstrando viabilidade para aplicações IoT em tempo real ($<$10~ms).}
...
O quinto experimento valida a aplicabilidade do sistema em cinco perfis representativos de dispositivos \ac{IoT}, cada um caracterizado por parâmetros físicos realistas de mobilidade, frequência de operação e condições de canal. A Figura~\ref{fig:exp05_perfis} apresenta graficamente o comportamento de \ac{BMR} e \ac{KDR} para todos os perfis, enquanto a Tabela~\ref{tab:exp05_perfis} sumariza os parâmetros e \ac{SNR} mínimo operacional.
...
\caption{Desempenho do sistema em cinco perfis de dispositivos IoT. (a) BMR inicial varia entre perfis de acordo com erro de estimação. (b) Todos os perfis atingem KDR $<1\%$ em 9--11~dB e KDR nula em 13~dB, demonstrando robustez desde sensores estáticos até veículos urbanos a 60~km/h.}
...
\caption{Descorrelação espacial: correlação de coeficientes de canal $\rho(h_{\text{Alice}}, h_{\text{Eve}})$ versus distância lateral (método Yuan et al.). A partir de 20~cm ($\approx 3\lambda/2$ a 2.4~GHz), a correlação torna-se desprezível ($\rho < 0.02$), garantindo segurança física contra espionagem passiva.}
\label{fig:exp06_eve}
\end{figure}
...
\caption{Segurança contra espionagem: BER de Eve versus guard-band (SNR = 15~dB). Eve mantém BER $\approx 50\%$ (chute aleatório) para todos os valores de GB, mesmo sem limiarização (GB=0), confirmando que a descorrelação espacial garante segurança física sem necessidade de técnicas adaptativas complexas.}
\label{fig:exp07_guardband}
\end{figure}

Esse resultado é particularmente relevante, pois demonstra que técnicas de limiarização adaptativa complexa, frequentemente propostas na literatura, não são necessárias para garantir a segurança do sistema quando a descorrelação espacial é adequadamente explorada. A Figura~\ref{fig:exp07_guardband} confirma que a \ac{BER} de Eve permanece próxima a $50\%$ independentemente do guard-band. Recomenda-se operar o sistema com GB $= 0$ ou GB $\leq 0.1\sigma$ para maximizar a taxa de geração de chaves sem comprometer a segurança.
