...
No cenário considerado, Alice e Bob recebem sinais \textit{downlink} provenientes de uma mesma estação rádio base, que transmite um sinal de referência $x$. Seguindo o modelo descrito em~\eqref{eq:canal_basico}, os sinais recebidos são expressos como $y_A = h_A \, x + n_A$ e $y_B = h_B \, x + n_B$, em que $h_A$ e $h_B$ denotam os coeficientes de canal entre a estação base e cada dispositivo, e $n_A, n_B \sim \mathcal{CN}(0,\sigma^2_n)$ representam ruído \ac{AWGN} independente em cada receptor.
...
\begin{enumerate}
    ...
    \item Recepção dos sinais \textit{downlink} conforme~\eqref{eq:canal_basico}: $y_A = h_A x + n_A$ e $y_B = h_B x + n_B$;
