...
Esta seção descreve o cenário físico e o modelo de comunicação adotado para a geração de chaves criptográficas em camada física. O objetivo central é possibilitar que dispositivos \ac{IoT} conectados a uma estação rádio base comum estabeleçam uma chave secreta compartilhada através da sondagem do canal \textit{downlink} e processos de reconciliação, viabilizando assim comunicação segura subsequente via criptografia simétrica. O sistema explora a correlação espacial entre os canais observados pelos dispositivos legítimos, enquanto garante segurança contra atacantes passivos por meio da descorrelação espacial natural.

\subsection{Cenário de Operação}

O sistema proposto opera no contexto de redes celulares de quinta geração (\ac{5G}) e tecnologias \ac{IoT}, em que múltiplos dispositivos estabelecem comunicação com uma estação rádio base (gNodeB). Considera-se especificamente o cenário em que dois dispositivos legítimos, denominados Alice e Bob, encontram-se espacialmente próximos (tipicamente a distâncias inferiores a 1~metro) e desejam estabelecer uma chave criptográfica compartilhada que lhes permitirá comunicar-se de forma confidencial.
...
Este trabalho adota o modelo de correlação espacial (dispositivos próximos observando canais \textit{downlink} correlacionados) em detrimento do modelo de reciprocidade temporal tradicional (exploração da reciprocidade do canal em sistemas TDD). A escolha justifica-se por quatro fatores principais. Primeiramente, a compatibilidade com sistemas \ac{5G} e \ac{NB-IoT} que frequentemente operam em modo FDD ou utilizam bandas assimétricas, impossibilitando exploração direta de reciprocidade temporal; o modelo espacial aplica-se independentemente da duplex. Em segundo lugar, a simplicidade de implementação: não é necessária sincronização temporal precisa nem transmissão de pilotos por Alice e Bob, bastando a recepção de sinais \textit{downlink} já disponíveis para estimação de canal em protocolos celulares padrão. Adicionalmente, a escalabilidade é naturalmente garantida, uma vez que múltiplos pares de dispositivos próximos podem gerar chaves simultaneamente sem interferência, utilizando os mesmos sinais de referência da estação base. Finalmente, a descorrelação espacial garante segurança física intrínseca contra atacantes distantes sem necessidade de suposições adicionais sobre o comportamento temporal do canal.
