...
variancias_ruido = potencia_sinal / (10 ** (snr_db_range / 10)) # variâncias do ruído para cada SNR
...
# Debug
logger.debug(f'Bits de informação gerados: {bits_informacao}')
logger.debug(f'Tamanho da cadeia de bits: {tamanho_cadeia_bits}')
logger.debug(f'Tamanho dos bits de informação: {tamanho_bits_informacao}')
...
for rayleigh_param in rayleigh_params:
    ...
    for variancia in variancias_ruido:
        ...
        snr_valores_db.append(snr_db)
        percentuais_acordo.append(kar)
    plot_kar(snr_valores_db, percentuais_acordo, rayleigh_param)
